...
| Social proof | The founder asked for review and tie-up sections on the home page. They exist, but **nothing on them is presented as real.** All content lives in `frontend/src/content/social-proof.ts`; every entry carries `placeholder: true`, and each band renders a visible "these are not real" notice while any entry still does. The notice clears itself as the flags come off — there is no second switch. `SHOW_SOCIAL_PROOF = false` hides both bands. **Do not delete these sections as a locked-decision violation, and do not quietly fill them with invented names.** The file's header comment carries the consent and trademark rules for replacing them. No `aggregateRating` JSON-LD, ever — `src/test/socialProof.test.tsx` locks that. |
