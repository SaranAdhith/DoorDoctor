...
def test_caregiver_cannot_change_the_medication_schedule(client, caregiver_headers):
    response = client.post(
        "/api/v1/patients/1/medications",
...
def test_administered_dose_is_logged(client, caregiver_headers, started_visit_id):
    response = client.post(
...
def test_skipped_dose_requires_a_reason(client, caregiver_headers, started_visit_id):
    response = client.post(
        f"/api/v1/visits/{started_visit_id}/medication-logs",
...
def test_refused_dose_requires_a_reason(client, caregiver_headers, started_visit_id):
    response = client.post(
        f"/api/v1/visits/{started_visit_id}/medication-logs",
...
def test_skipped_dose_with_a_reason_is_accepted(client, caregiver_headers, started_visit_id):
    response = client.post(
        f"/api/v1/visits/{started_visit_id}/medication-logs",
...
def test_logging_updates_adherence(client, caregiver_headers, family_headers, started_visit_id):
...
def test_relogging_the_same_medication_corrects_the_entry(client, caregiver_headers, family_headers, started_visit_id):
    client.post(
        f"/api/v1/visits/{started_visit_id}/medication-logs",
...
def test_medication_cannot_be_logged_before_checkin(client, caregiver_headers, scheduled_visit_id):
    response = client.post(
...
def test_medication_cannot_be_logged_after_completion(client, caregiver_headers, started_visit_id):
...
def test_adherence_reports_no_data_when_nothing_is_logged(client, db, coordinator_headers):
    ...
    response = client.get(
        f"/api/v1/patients/{patient.id}/medication-adherence", headers=coordinator_headers
    )
    assert response.status_code == 200
    body = response.json()
